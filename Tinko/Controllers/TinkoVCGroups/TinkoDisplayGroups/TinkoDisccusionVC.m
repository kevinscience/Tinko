...
@implementation TinkoDisccusionVC
...
//    User *theUser = [thisUser user];
//    if(theUser == nil){
//        NSLog(@"The User is nil, fetch data from firebase!");
//    }
    _db = FIRFirestore.firestore;
    NSString *facebookId = [[NSUserDefaults standardUserDefaults] objectForKey:@"facebookId"];
    FIRDocumentReference *myDocRef = [[_db collectionWithPath:@"Users"] documentWithPath:facebookId];
    ...
    Meet *meet = [_sharedMeet meet];
    _meetId = [_sharedMeet meetId];
    NSLog(@"TinkoDetail: %@, meetId: %@", meet.title, _meetId);
    //self.hidesBottomBarWhenPushed = NO;
    [super viewDidLoad];

    [self.tableView setSectionFooterHeight:100.0];
...
//    }
    ...
    FIRCollectionReference *messagesRef = [[[self.db collectionWithPath:@"Meets"] documentWithPath:_meetId] collectionWithPath:@"Messages"];
    FIRQuery *query = [messagesRef queryOrderedByField:@"postTime"];
...
- (void)didPressRightButton:(id)sender
{
    ...
    NSDictionary *messageDic = @{
                                 ...
                                 @"facebookId" : _theUser.facebookId,
                                 @"photoURL" : _theUser.photoURL,
                                 @"text" : [self.textView.text copy],
                                 @"postTime" : [NSDate date]
                                 };
    Message *message = [[Message alloc] initWithFullDataDictionary:messageDic];
    
    FIRCollectionReference *messagesRef = [[[self.db collectionWithPath:@"Meets"] documentWithPath:_meetId] collectionWithPath:@"Messages"];
    __block FIRDocumentReference *ref =
    [messagesRef addDocumentWithData:messageDic completion:^(NSError * _Nullable error) {
        if (error != nil) {
...
- (MessageTableViewCell *)messageCellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.thumbnailView sd_setImageWithURL:[NSURL URLWithString:message.photoURL]
